'use client';

import { DataTable } from '@/components/super-admin/DataTable';

export default function FeedbackPage() {
  const columns = [
    {
      key: 'rating',
      header: 'Rating',
      render: (row: any) => (
        <div className="flex items-center">
          {[...Array(5)].map((_, i) => (
            <svg
              key={i}
              className={`w-4 h-4 ${i < row.rating ? 'text-yellow-400' : 'text-gray-300'}`}
              fill="currentColor"
              viewBox="0 0 20 20"
            >
              <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
            </svg>
          ))}
        </div>
      ),
    },
    { key: 'type', header: 'Type' },
    {
      key: 'subject',
      header: 'Subject',
      render: (row: any) => (
        <div>
          <p className="font-medium">{row.subject}</p>
          <p className="text-sm text-gray-500 truncate max-w-xs">{row.message}</p>
        </div>
      ),
    },
    {
      key: 'student_name',
      header: 'Student',
      render: (row: any) => row.students?.name || 'Anonymous',
    },
    {
      key: 'branch_name',
      header: 'Branch',
      render: (row: any) => row.branches?.name || 'N/A',
    },
    { key: 'created_at', header: 'Date' },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Feedback Analytics</h1>
        <p className="text-gray-600">View and analyze feedback from all branches</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <p className="text-sm text-gray-500">Total Feedback</p>
          <p className="text-2xl font-bold text-gray-900">0</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <p className="text-sm text-gray-500">Average Rating</p>
          <p className="text-2xl font-bold text-gray-900">0.0</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <p className="text-sm text-gray-500">Positive Reviews</p>
          <p className="text-2xl font-bold text-gray-900">0</p>
        </div>
      </div>

      <DataTable columns={columns} data={[]} />
    </div>
  );
}
